...
3. **Authenticate everything.** Every HTTP endpoint and every SignalR connection must require
   ...
   `/alive`, `/redis-check`, the login/signup endpoints, `GET /v1/players`, and `GET /v1/games` — the
   web dashboard shows the player pool and the NBA schedule to signed-out visitors, and nothing
   either returns is user-specific.
